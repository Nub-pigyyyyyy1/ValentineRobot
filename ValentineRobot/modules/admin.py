...
from ValentineRobot import DRAGONS, dispatcher, telethn as bot
from ValentineRobot.modules.disable import DisableAbleCommandHandler
from ValentineRobot.modules.helper_funcs.chat_status import (
    bot_admin,
    can_pin,
    can_promote,
    connection_status,
...
from ValentineRobot.modules.helper_funcs.extraction import (
    extract_user,
    extract_user_and_text,
)
from ValentineRobot.modules.log_channel import loggable
from ValentineRobot.modules.helper_funcs.alternate import send_message
    ...
    if msg.reply_to_message:
        ...
        try:
            ...
            if excp.message == "Participants_too_few":
                return msg.reply_text(
                    "Sorry f*cker, due to telegram restrictions chat needs to have minimum 100 members before they can have group stickers!"
                )
            msg.reply_text(f"Error! {excp.message}.")
    else:
        msg.reply_text("You need to reply to some sticker to set chat sticker set. dumb f*cker!!")
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("I can't promote myself! Get an admin to do it for me.f*cker! ")
        return
...
async def middemote(dmod):
    ...
    if dmod.is_group:
        if not await is_register_admin(dmod.input_chat, user.id):
            await dmod.reply("**f#cker, i cant demote non-admin**")
            return
    else:
        return
