from ValentineRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from ValentineRobot import DRAGONS
...
async def Valentine_is_admin(chat_id: int):
    status = False
    Valentine = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if Valentine.id == user.id:
            status = True
            break
    return status
